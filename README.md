...
<img src="https://skillicons.dev/icons?i=python,r,c,cpp,javascript,bash,git,github,linux,docker" />

</p>

`Python` `R` `C` `C++` `JavaScript` `Bash` `Git` `GitHub` `Linux` `Docker`
...
`LSTM` `GRU` `Transformers` `Scikit-learn` `PyTorch` `TensorFlow`
...
Interested in scalable computing for **climate modeling, geospatial analysis, and environmental data processing**.
...
`Arduino` `Raspberry Pi` `IoT` `Environmental Sensors`
